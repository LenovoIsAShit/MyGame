...
public class detect_system : MonoBehaviour
{
    ...
    public void Reset_detected()
    {
        detected = false;

        BehaviorTree bt = GetComponent<BehaviorTree>();
        bt.GetVariable("detected").SetValue(false);
        bt.EnableBehavior();
        Change_attack_state(false, Color.blue);
    }
    ...
    public void Change_attack_state(bool dis,Color color)
    {
        Set_detected_image(dis);

        Set_red_eye_light(color);
    }
    //改变为发现敌人状态
    ...
    void Detect_check()
    {
        ...
        {
            Change_attack_state(true,Color.red);
    ...
    void Set_detected_image(bool chi)
    {
        detected_image.gameObject.SetActive(chi);
    }//设置感叹号图片可见

    void Set_red_eye_light(Color color)
    {
